...
# Use config values
db_path = config.portal_db_path
print(f"Using database path:", db_path)

# Configuration
ROOT_DIR = os.path.dirname(os.path.abspath(__file__))
DATA_DIR = config.data_dir or os.getenv('RESETTE_DATA_DIR', os.path.join(ROOT_DIR, "data"))
STATIC_DIR = config.static_dir or os.getenv('RESETTE_STATIC_DIR', os.path.join(ROOT_DIR, "static"))
PORTAL_DB_PATH = db_path or os.path.join(DATA_DIR, "portal.db")
...
def verify_migration():
    ...
    print("Verifying migration. Using database at:", PORTAL_DB_PATH)
    if not os.path.exists(PORTAL_DB_PATH):
